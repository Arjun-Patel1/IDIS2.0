...
st.title("🧠 Intelligent Document Intelligence System (IDIS)")
st.caption(
    "AI-powered customer support issue classification using "
    "ML + RAG + Self-Healing Feedback"
)
...
if analyze_btn and user_input.strip():
    ...
    if response.status_code != 200:
        ...
        col1, col2, col3 = st.columns(3)
        col1.metric("🏷️ Category", data["predicted_category"])
        col2.metric("📊 Confidence", round(data["confidence"], 2))
        col3.metric("🧮 Calibrated Confidence", data["calibrated_confidence"])
        ...
        # ----------------------------------
        # RAW JSON (for recruiters/debug)
        # ----------------------------------
        with st.expander("🔍 View Raw JSON Output"):
            st.code(json.dumps(data, indent=2))
if st.button("👍 Mark Explanation as Correct"):
